...
> - Nunca incluyas secretos, credenciales, cookies, tokens, claves, datos personales, hostnames o IPs internas, endpoints privados, nombres de proyectos internos ni detalles de despliegue. Redacta los logs y la configuración antes de publicarlos. Para vulnerabilidades, consulta [`SECURITY.md`](SECURITY.md).
...
Al contribuir, aceptas que tu código se publique bajo la **licencia declarada en el repositorio correspondiente**. No se presupone una licencia OSS: consulta el `README` y el archivo `LICENSE` de ese repositorio.
